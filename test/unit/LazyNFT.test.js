  ...
  : describe("LazyNFT Unit Tests", () => {
      ...
      describe("LazyNFT Constructor is set correctly", () => {
        ...
        it("sets the creatorRole to deployer", async () => {
          ...
          expect(await LazyNft.hasRole(creatorRole, deployer.address)).to.be
            .true;
        });
      });
      ...
      describe("LazyNFT redeem", () => {
        it("mints an NFT for a valid voucher", async () => {
          ...
          const redeemTransaction = await LazyNftPlayer.redeem(
            player.address,
            voucher,
            {
          ...
          const playerLazyNftBalance = await LazyNftPlayer.balanceOf(
            player.address
          );
        ...
        it("reverts if the voucher is tampered with", async () => {
          ...
          await expect(
            LazyNft.redeem(player.address, tamperedVoucher)
          ).to.be.revertedWith("LazyNFT__UnauthorizedCreator");
        });
      });
    });
